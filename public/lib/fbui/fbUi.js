...
;(function(window, $) {
    ...
    fbUi.waterfall = function(options){
		...
		var fb = $.extend(defaults, options || {});
		var $el = $(fb.el);
		var $el_width = $el.parents(".pageMain").width();
		
		fb.column = parseInt($el_width/(fb.width+fb.margin)) > fb.maxColumn ? fb.maxColumn : parseInt($el_width/(fb.width+fb.margin)) ;
		console.log($el_width,fb.width)
		$el.css({"width":fb.column*(fb.width+fb.margin)-fb.margin,"margin":"30px auto"})
		...
		$.each(fb.data,function(a,b){
			...
			console.log(parseFloat(b.width),fb.margin)
		...
		if(fb.textBox == 'input'){
			var html = '<div style="display:none;" class="fb-showModal fb-position-fixed fb-z999 fb-showModal-black" >\
				        <div class="fb-showModal-group fb-position-relative fb-transition startTop" >\
				            ...
				            <div class="fb-showModal-content fb-font14"><input class="fb-textBox  type="text" placeholder="'+fb.placeholder+'" value="'+fb.value+'" /></div>\
			...
			var html = '<div style="display:none;" class="fb-showModal fb-position-fixed fb-z999 fb-showModal-black" >\
				        <div class="fb-showModal-group fb-position-relative fb-transition startTop" >\
				            <div class="fb-showModal-title fb-font18">'+fb.title+'</div>\
				            <div class="fb-showModal-content fb-font14"><textarea class="fb-textBox  placeholder="'+fb.placeholder+'"  >'+fb.value+'</textarea></div>\
				            <div class="fb-showModal-buttom ">\
				                <button class="fb-showModal-close fb-inlineBlock fb-font16">取消</button>\
				                <button class="fb-showModal-true fb-buttonFill-info fb-inlineBlock fb-font16" >确定</button>\
				            </div>\
		...
		$el.on("submit",function(){
			...
			$.each(fb,function(a,b){
				...
				if(b.rules){
					...
					switch(b.rules) {
						case 'required':
							var val = $el_this.find("[name=\""+b.name+"\"]");
							...
							break;
						case 'is_email':
							var val = $el_this.find("[name=\""+b.name+"\"]").val();
						...
						case 'is_equal':
							var val = $el_this.find("[name=\""+b.name+"\"]").val();
							var equalVal = $el_this.find("[name=\""+b.equalName+"\"]").val();
